...
/* ---------------------------------------------------
	...
	Action   : 
	creer une zone de memoire partagee grace
    ...
    emplit la structure zone passee en argument
 --------------------------------------------------- */
...
int supprimerZonePartagee(Zone * zp){
	...
	if (!zp) { fprintf(stderr,"Erreur: zp==NULL dans supprimerZonePartagee\n");
 	...
 	if (shmctl(zp->id, IPC_RMID, NULL)==-1) { 
		// le flag IPC_RMID indique que le shm sera marqué pour la suppression. Il sera supprimé dès que plus aucun processus
		// n'y sera attaché
